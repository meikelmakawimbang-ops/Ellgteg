...
// Auto read & compose (mengetik) to make bot more realistic
try {
    if (m.chat && !m.fromMe && m.chat !== 'status@broadcast') {
        await this.readMessages([m.key])
        await this.sendPresenceUpdate('composing', m.chat)
        
        // Auto react to incoming message with a random emoji
        const emojis = ['👍', '🙌', '👏', '🙏', '❤️', '🔥', '✨', '😊', '😂', '🤣', '😭', '😎', '👀', '💯'];
        const randomEmoji = emojis[Math.floor(Math.random() * emojis.length)];
        await this.sendMessage(m.chat, { react: { text: randomEmoji, key: m.key } });
    }
} catch (e) {
    console.error('Presence/Read/React error:', e)
}
...
let isSpam = false
if (global.db.data.users[m.sender] && new Date() - (global.db.data.users[m.sender].spam || 0) < 3000) {
    isSpam = true
}
...
let pp = global.gataImg || 'https://telegra.ph/file/2403590c5d6a3d1aa1a01.jpg'
try {
pp = await this.profilePictureUrl(user, 'image')
} catch (e) {
console.log('Using fallback avatar for', user)
}
let apii = { data: Buffer.alloc(0) }
try {
apii = await this.getFile(pp)
} catch (e) {
console.error('Error fetching file for profile picture:', e)
}
